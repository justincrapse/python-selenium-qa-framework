...
class BPBaseTestCase(BaseTestCase):
    @staticmethod
    def wa_login(tc: BPTestContext, with_unique_email=False, password_override=None, email_override=None):
        if with_unique_email:
            new_email = BPBaseTestCase.generate_unique_email(tc)
            __class__.set_unique_email(tc, unique_email=new_email)
        login_page = bp_pages.LoginPage(tc).navigate()
        login_page.log_in(
    ...
    @staticmethod
    def wa_logout(tc):
        account_menu = bp_pages.AccountSubNav(tc).navigate()
        account_menu.btn_log_out.click()

    @staticmethod
    def generate_unique_email(tc: BPTestContext, suffix_length=6, from_session_id=True):
    ...
    @staticmethod
    def set_unique_email(tc: BPTestContext, unique_email):
        tc.user['UNIQUE_EMAIL'] = unique_email

    @staticmethod
    def clear_cart(tc):
        nav_bar = bp_pages.NavBar(tc).navigate()
        cart_page = bp_pages.CartPage(tc)
